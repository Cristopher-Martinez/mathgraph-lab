...
export default function PracticePage() {
  ...
  if (view === "socratic" && currentEx) {
    // Los pasos se obtienen del backend en startSession, inicializar con los que haya
    const socraticSteps =
  ...
  if (view === "solving" && currentEx) {
    ...
    const topicFormulas = currentEx.topic?.formulas || [];
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 mb-4">
            <span className="px-2 py-0.5 rounded text-xs bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 font-medium">
              {currentEx.topic?.name || filter.topicName}
            </span>
